...
mpz_class hamming(const mpz_class& n, const int& primeindex)
{
    ...
    else
    {
        ...
        std::cout << "h(" << n << "," << primeindex << ") = " << ret << std::endl;
        hammingcache.insert(std::pair<hammingcachekey, mpz_class>(hck, ret));
        return ret;
    }
}

void precalc(const mpz_class& n, const int& primeindex)
{
    for (int pi = 2; pi < primeindex; pi++)
    {
        std::cout << ":" << pi << " / " << primes.size() << "     \r";
        hamming(n, pi);
    }
}
...
std::pair<mpz_class, bool> hamming_cs(const hammingcachekey& k)
{
    mpz_class result;
    if (k.first <= 1)
        result = k.first;
    else if (k.second == 0)
        result = mpz_sizeinbase(k.first.get_mpz_t(), 2);
    else
    {
        auto cached = hammingcache.find(k);
        if (cached == hammingcache.end())
        {
            return std::pair<mpz_class, bool>(result, false);
        }

        result = cached->second;
    }
    return std::pair<mpz_class, bool>(result, true);
}
...
std::pair<mpz_class, bool> trycalc(const hammingcachekey& k)
{
    auto existing = hamming_cs(k);
    if (existing.second) return existing; // not expecting this, but if it is, we'll take it
    hammingcachekey l(k.first / primes[k.second], k.second);
    hammingcachekey r(k.first, k.second - 1);
    std::pair<mpz_class, bool> lresult, rresult;
    if ((lresult = hamming_cs(l)).second && (rresult = hamming_cs(r)).second)
    {
        hammingcache.erase(l);
        hammingcache.erase(r);
        return std::pair<mpz_class, bool>(lresult.first + rresult.first, true);
    }
    else
    {
        return std::pair<mpz_class, bool>(mpz_class(0), false);
    }
}

int it = 0;
mpz_class hamming_w(const mpz_class n, int p)
{
    hammingcachekey current(n, p);
    std::set<hammingcachekey> worklist;
    worklist.insert(current);
    while (true)
    {
        if (it++ % 100000 == 0) std::cout << "Worklist: " << worklist.begin()->first << "," << worklist.begin()->second << ", " << worklist.size() << ", cache: " << hammingcache.size() << "                  \r";

        auto calc_result = trycalc(*worklist.begin());
        if (calc_result.second)
        {
            if (worklist.begin()->first == n && worklist.begin()->second == p)
            {
                return calc_result.first;
            }
            if (worklist.begin()->first > 1 && worklist.begin()->second > 0) // don't need to cache trivial values, as they can be given immediately by hamming_cs
            {
                hammingcache.insert(std::pair<hammingcachekey, mpz_class>(*worklist.begin(), calc_result.first));
            }
            worklist.erase(worklist.begin());
        }
        else
        {
            hammingcachekey l(worklist.begin()->first / primes[worklist.begin()->second], worklist.begin()->second);
            hammingcachekey r(worklist.begin()->first, worklist.begin()->second - 1);
            worklist.insert(l);
            worklist.insert(r);
        }
#if _DEBUG
        std::cout << "WL: ";
        for (auto k : worklist) std::cout << k.first << "," << k.second << "   ";
        std::cout << "Cache: ";
        for (auto k : hammingcache) std::cout << k.first.first << "," << k.first.second << "   ";
        std::cout << std::endl;
#endif
    }
}

void test(const mpz_class& n, int i)
{
    auto res = hamming_w(n, i);
    std::cout << std::endl << "H(" << n << "," << i << ") = " << res << std::endl;
}
