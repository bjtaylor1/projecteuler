...
cube::cube(const vector3d & U, const vector3d & V, const vector3d & N) : uvn({U,V,N}), width(0), height(0), depth(0)
{
	...
	for (vector<vertex>::const_iterator it = tempvertices.begin(); it != tempvertices.end(); it++)
	{
		...
		width = max(width, (bigulong)v.x);
		height = max(height, (bigulong)v.y);
		depth = max(depth, (bigulong)v.z);
		vertices.insert(v);
	}
...
void cube::add_vertex(vector<vertex>& vertices, biglong& minx, biglong& miny, biglong& minz, const vertex& vertex)
{
